...
<title>Data Member — LEARNBASE.</title>
...
body.dark-mode .sidebar, body.dark-mode .topbar, body.dark-mode .page-card, body.dark-mode .table-wrap { background: var(--card-bg); }
body.dark-mode .sidebar, body.dark-mode .topbar, body.dark-mode .page-card, body.dark-mode .table-wrap { border-color: var(--line); }
...
.search-bar input {
  flex: 1; max-width: 380px; padding: 0.6rem 1rem; border: 2px solid var(--line); border-radius: 12px;
  font-size: 0.88rem; font-family: 'Inter', sans-serif; color: var(--charcoal);
  background: var(--bg); transition: border-color .2s;
}
.search-bar input:focus { border-color: var(--deep-green); outline: none; }
.search-bar button {
  padding: 0.6rem 1.25rem; border-radius: 12px; border: none;
  background: var(--deep-green); color: #fff; font-weight: 600; cursor: pointer;
}
...
.btn-sm.outline { background: transparent; border: 1px solid var(--line); color: var(--gray); text-decoration: none; display: inline-flex; }
...
<div class="main-area">
  <header class="topbar">
    <div class="topbar-title">Data Member</div>
  ...
  <div class="wrap">
    ...
    <div class="search-bar">
      <form method="get" action="<?= site_url('admin/search_members') ?>" style="display:flex;gap:0.5rem;width:100%;">
        <input type="text" name="q" placeholder="Cari nama atau email member..." value="<?= $search_query ?? '' ?>">
        <button type="submit">Cari</button>
        <?php if (isset($search_query) && !empty($search_query)): ?>
        <a href="<?= site_url('admin/members') ?>" class="btn-sm outline" style="padding:0.6rem 1rem;">✕ Reset</a>
        <?php endif; ?>
      </form>
    </div>
    ...
    <div class="table-wrap">
      <table>
        ...
        <tbody>
          ...
          <tr><td colspan="5" style="text-align:center;color:var(--gray-soft);padding:2rem;"><?= isset($search_query) ? 'Member tidak ditemukan.' : 'Belum ada member.' ?></td></tr>
          <?php else: ?>
          <?php foreach ($members as $m): ?>
          <tr>
            <td><strong><?= $m['nama'] ?></strong></td>
            <td><?= $m['email'] ?></td>
            <td><span class="badge <?= $m['membership'] ?>"><?= ucfirst($m['membership']) ?></span></td>
            <td><?= date('d M Y', strtotime($m['created_at'])) ?></td>
            <td>
              <a href="<?= site_url('admin/member_detail/' . $m['id']) ?>" class="btn-sm primary">📊 Detail</a>
              <a href="<?= site_url('admin/grant_premium/' . $m['id']) ?>" class="btn-sm <?= $m['membership'] === 'premium' ? 'warning' : 'primary' ?>" style="<?= $m['membership'] === 'premium' ? 'background:var(--orange-light);color:var(--orange);' : '' ?>"><?= $m['membership'] === 'premium' ? '⬇ Revoke Premium' : '⭐ Grant Premium' ?></a>
              <a href="<?= site_url('admin/delete_member/' . $m['id']) ?>" class="btn-sm danger" onclick="return confirm('Yakin ingin menghapus akun <?= $m['nama'] ?>? Semua data progres akan ikut terhapus.')">🗑️ Hapus</a>
            </td>
          </tr>
          <?php endforeach; ?>
          <?php endif; ?>
