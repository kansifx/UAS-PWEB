...
<title>Kelola Modul — LEARNBASE.</title>
...
.page-title { font-weight: 700; font-size: 1.4rem; margin-bottom: 1.25rem; }
...
.badge.beginner { background: var(--deep-green-light); color: var(--deep-green); }
.badge.intermediate { background: var(--orange-light); color: var(--orange); }
.badge.advanced { background: #F0EFED; color: var(--charcoal); }
.btn-sm { display: inline-flex; align-items: center; gap: 0.3rem; padding: 0.35rem 0.85rem; border-radius: 8px; font-size: 0.78rem; font-weight: 600; border: none; cursor: pointer; text-decoration: none; transition: background .15s; }
...
<div class="main-area">
  <header class="topbar">
    <div class="topbar-title">Kelola Modul</div>
  ...
  <div class="wrap">
    <h1 class="page-title">�� Daftar Modul Pembelajaran</h1>
    <div class="table-wrap">
      <table>
        <thead><tr><th>Modul</th><th>Kategori</th><th>Level</th><th>Slug</th><th>Aksi</th></tr></thead>
        <tbody>
          <?php foreach ($modules as $m): ?>
          <tr>
            <td><strong><?= $m['name'] ?></strong></td>
            <td><?= $m['category'] ?></td>
            <td><span class="badge <?= $m['difficulty'] ?>"><?= ucfirst($m['difficulty']) ?></span></td>
            <td><?= $m['slug'] ?></td>
            <td><a href="<?= site_url('mentor/edit_module/' . $m['slug']) ?>" class="btn-sm primary"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:middle;margin-right:4px;"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg> Edit</a></td>
          </tr>
          <?php endforeach; ?>
        </tbody>
      </table>
